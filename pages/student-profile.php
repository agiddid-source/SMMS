...
// Bursar & Payments: fees this student can be paid against (for the record
// modal), and — when we arrived here via the handler's ?receipt= redirect —
// the payment to show as a printable receipt.
$profile_fees = pay_get_outstanding_fees($student['id']);
$profile_receipt_id = cure($_GET['receipt'] ?? '');
$profile_receipt = $profile_receipt_id !== '' ? pay_find_payment($profile_receipt_id) : null;
$profile_receipt_student = $profile_receipt ? pay_get_student($profile_receipt['studentId']) : null;
?>
<?php render_toast(); ?>
      ...
      <section class="ght-card t-resize ght-profile-header">
        <div class="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div class="flex flex-wrap items-center gap-3 xl:justify-end">
            ...
            <a class="ght-button ght-button--secondary text-sm font-medium" href="index.php?p=invoices&student=<?= urlencode($student['id']) ?>">
...
<?php
// Bursar record-payment modal (opened by the "Record payment" label above) and,
// after a record redirects back with ?receipt=, the printable receipt. Both are
// the shared Bursar components, so this profile and the payments desk record
// payments the exact same way.
$pay_form_student = $student;
$pay_form_fees = $profile_fees;
$pay_form_return_to = 'student-profile';
$pay_form_modal_id = 'pay-record-modal';
require __DIR__ . '/../components/pay-payment-form.php';

if ($profile_receipt && $profile_receipt_student):
    $pay_receipt_payment = $profile_receipt;
    $pay_receipt_student = $profile_receipt_student;
    $pay_receipt_done_url = 'index.php?p=student-profile&student=' . urlencode($student['id']);
