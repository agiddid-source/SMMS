...
<form id="<?= $form_id ?>" method="post" action="index.php?p=discounts">
  ...
  <div class="cn-field-grid">
    ...
    <div class="cn-field"><label>Applies to</label><select name="appliesTo" class="cn-input" required><option value="tuition" <?= ($discount['appliesTo'] ?? '') === 'tuition' ? 'selected' : '' ?>>Tuition only</option><option value="total_fees" <?= ($discount['appliesTo'] ?? '') === 'total_fees' ? 'selected' : '' ?>>Total school fees</option></select></div>
  </div>
